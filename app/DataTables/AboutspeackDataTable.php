...
class AboutspeackDataTable extends DataTable
{
    /**
    ...
    public function checkrights($row)
    {
        ...
        // $historyurl = route('attributes.history', [$row->id]);
            ...
            <div class="dropdown-menu dropdown-menu-right shadow animated--fade-in" aria-labelledby="dropdownMenuLink">
                <a class="dropdown-item" href="' . route('aboutspeack.edit', $row->id) .'"><svg viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg" width="24" height="24" fill="none" viewBox="0 0 24 24">
  <style>
    .rotate-right {
...
@keyframes rotate-right {
 ...
 Edit</a>
                ...
                <form action=" '. route('aboutspeack.destroy', $row->id) .' ")" method="POST">
                '.csrf_field().'
                '.method_field("DELETE").'
     ...
     *
     * @param \App\Models\Aboutspeack $model
     * @return \Illuminate\Database\Eloquent\Builder
     */
    public function query(Aboutspeack $model)
    {
        $model = Aboutspeack::select();
        ...
        if (request()->get('position', false)) {
            $model->where('position', 'like', "%" . request()->get("position") . "%");
        }
    ...
    {
        return 'Aboutspeack_' . date('YmdHis');
    }
}

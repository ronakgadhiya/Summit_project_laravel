...
class UserDataTable extends DataTable
{
        ...
        return datatables()
            ...
            ->editColumn('city', function($row){
                // dd($state);
                 if(isset($row->city[0]))
                 {
                     return  $row->city[0]['name'];
                 }
                 else{
 
                     return 'its not have city';
                 }
             })
    ...
    public function checkrights($row)
    {
        ...
        // $historyurl = route('attributes.history', [$row->id]);
            ...
            <div class="dropdown-menu dropdown-menu-right shadow animated--fade-in" aria-labelledby="dropdownMenuLink">
                <a class="dropdown-item" href="' . route('users.edit', $row->id) .'"><svg viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg" width="24" height="24" fill="none" viewBox="0 0 24 24">
                <style>
                  .rotate-right {
    ...
    {   
            $model = User::select()->where('email', '!=', 'ronakgadhiya3@gmail.com'); 
            
            if (request()->get('f_name', false)) {
                $model->where('f_name', 'like', "%" . request()->get("f_name") . "%");
            }
            if (request()->get('l_name', false)) {
                $model->where('l_name', 'like', "%" . request()->get("l_name") . "%");
            }
            ...
            if (request()->get('mobile', false)) {
                $model->where('mobile', 'like', "%" . request()->get("mobile") . "%");
            } 
            if (request()->get('address', false)) {
                $model->where('address', 'like', "%" . request()->get("address") . "%");
            }
            ...
            if (request()->get('city', false)) {
                $model->where('city_id',request()->get("city"));
            }

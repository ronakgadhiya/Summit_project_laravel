...
class TiketDataTable extends DataTable
{
    ...
    public function dataTable($query)
    {
        return datatables()
            ...
            ->editColumn('username', function($row){
                if(isset($row->User[0]))
                {
                    return  $row->User[0]['username']; //$row->user[0] user=>user name nu relation time je function apel hoi ee rakhvu
                }
                else{

                    return 'its not have username';
                }
            })
            ->editColumn('feature', function($row){
                //
                if(isset($row['featureName'][0]))
                {
                    return  $row['featureName'][0]['name']; //$row->user[0] user=>user name nu relation time je function apel hoi ee rakhvu
                }
                else{

                    return 'its not have feature';
                }
            })
    ...
    public function checkrights($row)
    {
        ...
        // $historyurl = route('attributes.history', [$row->id]);
            ...
            <div class="dropdown-menu dropdown-menu-right shadow animated--fade-in" aria-labelledby="dropdownMenuLink">
                <a class="dropdown-item" href="' . route('tiket.edit', $row->id) .'"><svg viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg" width="24" height="24" fill="none" viewBox="0 0 24 24">
            <style>
                .rotate-right {
            ...
            Edit</a>
                ...
                <form action=" '. route('tiket.destroy', $row->id) .' ")" method="POST">
                '.csrf_field().'
                '.method_field("DELETE").'
     ...
     *
     * @param \App\Models\Tiket $model
     * @return \Illuminate\Database\Eloquent\Builder
     */
    public function query(Tiket $model)
    {
        $model = Tiket::select()->with(['User','featureName']);
        if (request()->get('username', false)) {
            $model->where('user_id',request()->get("username"));
        }
        if (request()->get('title', false)) {
            $model->where('title', 'like', "%" . request()->get("title") . "%");
        }
        if (request()->get('subtitle', false)) {
            $model->where('subtitle', 'like', "%" . request()->get("subtitle") . "%");
        } 
        if (request()->get('btntitle', false)) {
            $model->where('btntitle', 'like', "%" . request()->get("btntitle") . "%");
        }
        if (request()->get('feature', false)) {
            $model->where('feature', 'like', "%" . request()->get("feature") . "%");
        }
        if (request()->get('price', false)) {
            $model->where('price', 'like', "%" . request()->get("price") . "%");
        }
